...
write_rank(rank.s1, "rank1.csv")
rank.s2 <- AutoScore::AutoScore_rank(dat2_split$train)
write_rank(rank.s2, "rank2.csv")
# then get global rank:
rank.fed <- AutoScore_Frank(method=1) # by using results generated by RF at each site
# write.csv(rank_fed, "allRanks.csv")
...
# plot 1
df %>% group_by(Model) %>% summarize(AUC_mean = mean(AUC)) %>% ggplot(aes(x=Model, y = AUC_mean)) +  geom_line() + geom_point()
# plot 2
...
# based on the parsimony plot, we may select the first 6 variables for final model
